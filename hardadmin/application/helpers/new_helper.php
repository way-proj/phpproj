...
function getProductbycat($table,$colum,$where){
  $CI = &get_instance();
  $CI->db->select($colum);
...
//Header product
/*function getProductData($cid){
       	$CI = &get_instance(); 
		$CI->db->select('*');
		$CI->db->from('tbl_products AS p'); 
		$CI->db->join('tbl_product_attribute pa', 'pa.product_id=p.id', 'left');
		$CI->db->join('product_attribute_image pi', 'pi.attr_id=pa.id', 'left');
		//$this->db->order_by("product_attribute_image.created_date","desc");
		$CI->db->where('cat_id',$cid);
		$query = $CI->db->get(); 
...
function getProductData($cid){
		...
		$query = $CI->db->get();  */
		 ...
		 $CI->db->select("tbl_products.*,tbl_product_attribute.id as attr_id,product_attribute_image.image");
         $CI->db->from("tbl_products");
         $CI->db->join("tbl_product_attribute",'tbl_product_attribute.product_id=tbl_products.id');
         
		 $CI->db->join('product_attribute_image', 'product_attribute_image.attr_id=tbl_product_attribute.id', 'left');
		 
		 $CI->db->group_by("tbl_product_attribute.product_id","asc");
         $CI->db->where('cat_id',$cid);
		$query = $CI->db->get();
		//echo $this->db->last_query(); die;
		if($query->num_rows() != 0)
	...
	function getbrand($id,$table,$colum){
