...
public function set_status(){
       ...
       $order_id=array('WAYINFO-20210128-172','WAYINFO-20210128-171');
       $row = $this->Product_model->order_list_details($order_id);

       $delivery_boy_id=$this->input->post('delivery_boy_id');
       $get_tocken = $this->Product_model->get_tocken($delivery_boy_id);
       //@$get_tocken->device_token='ExponentPushToken[jtGLUSKERblWCNkUIdINoG]';
     if(@$get_tocken->device_token!=''){
        
        // $address=$row->address1;
        // $mobile=$row->phone;
        // $payment_mode=$row->payment_option;
        // $sale_id=$row->sale_id;
        // $order_id=$row->province;
         $token=$get_tocken->device_token;
...
public function cod_orders(){
     ...
     $data['all_delivery_user']=$this->Product_model->get_all_delivery_user();
    ...
    //    } 
      ...
      $upload_file=$this->do_uploads();
	   ...
	   $this->load->library('upload');
			...
			//brochure	
			$file_upload_name1="";
			if(isset($_FILES['brochure_file']) && $_FILES['brochure_file']['name'] != '')
				{
				    $config=array();
				    $config['upload_path'] = getcwd().'/upload/brochure/';
				//$config['allowed_types'] = 'jpg|png|jpeg|gif|webp';
				 $config['allowed_types'] = 'application/pdf|pdf|application/octet-stream|csv';
					...
					$this->form_validation->set_rules('brochure_file', 'brochure file','required',array('required'=>$uploadStatus));
      ...
      $insert_data=array(
        ...
        'industrytype'=>$this->input->post('industry_type'),
		'brochure_file'=>$file_upload_name1,
     ...
     // die;
      if($eid){
        ...
        $this->Product_model->update_attribute_product_data($upload_file,$_POST,$eid);
         ...
         $last_id=$this->Product_model->save_product($insert_data,$_POST); 
...
function do_uploads(){
 ...
 //echo'<pre>';print_r(@$file_datas);die;
  ...
  /*public function addpromaster(){
        ...
        $data['industrytype']=$this->Product_model->industrytype();
        $eid=$this->uri->segment(3);
       if($eid){
		...
		$data['long_desc_data']=$this->Product_model->Get_long_desc($eid);
		//$data['single_industrytype']=$this->Product_model->single_industrytype($eid);
         }
        $data['cat_data']=$this->Product_model->cat_list();
   ...
   */
   public function addpromaster(){
       ...
       if($eid){
        $data['single_cat_data']=$this->Product_model->single_child_cat_list($eid);
        $data['single_pro_data']=$this->Product_model->single_pro_data($eid);
		//print_r($data['single_pro_data']);die;
		$data['long_desc_data']=$this->Product_model->Get_long_desc($eid);
		$data['single_industrytype']=$this->Product_model->single_industrytype($eid);
         }
        $data['cat_data']=$this->Product_model->cat_list();
        ...
        $data['sub_child_cat_data']=$this->Product_model->sub_sub_cat_lists();
        //$data['get_attr']=$this->Product_model->get_attr_list($eid);
		$data['get_attr']=$this->Product_model->get_attribute_details($eid);
        $data['only_prodcuct']=$this->Product_model->get_product();
        $data['subview']="add-product";
        $this->load->view('layout/default',$data);
