...
class Product_model extends CI_Model{
    function get_tocken($delivery_boy_id){
            $this -> db -> select(' * ');
...
public function order_list_details($order_id){
       $this->db->select("tbl_checkout.*");
       $this->db->where_in('tbl_checkout.province',$order_id);
       $this->db->from("tbl_checkout");
       $query=$this->db->get();
      // echo $this->db->last_query();die;
       $res=$query->result();
...
function get_product(){
   ...
   /* public function save_product($insert_data){
    ...
    public function save_product($insert_data,$post_data){
        /*  $this->db->insert('tbl_products',$insert_data);
        $insert_id=$this->db->insert_id();
        return $insert_id;  */
		...
		$text3=@$post_data['text3'];
		$text4=@$post_data['text4'];
		$text5=@$post_data['text5'];
		$text6=@$post_data['text6'];
		$text7=@$post_data['text7'];
		$text8=@$post_data['text8'];
		$text9=@$post_data['text9'];
		$text10=@$post_data['text10'];
		$text11=@$post_data['text11'];
		$text12=@$post_data['text12'];
        ...
        for($i=0;$i<$cnt;$i++){
           ...
           $this->db->insert('product_gallery',$insert_data); 
             ...
             //print_r($insert_data);
            $this->db->insert('product_long_desc',$insert_data); 
  ...
  public function update_attribute_product_data($upload_file,$post_data,$eid){
    ...
    $itemcode=@$post_data['itemcode'];
	$industrytype=@$post_data['industrytype'];
    $color=@$post_data['color'];
    $size=@$post_data['size'];
    $weight=@$post_data['weight'];
    $storage=@$post_data['storage'];
        			  ...
        			  //lid
        			    ...
        			    if($lid)
        			    {
        			     $this->db->where('id',$lid);
                          ...
                          $this->db->update('product_long_desc',$update_data);  
        			    }
        			    else
        			    {
        			        $insert_data=array(
...
FROM
  ...
  LEFT JOIN `tbl_products` AS tp 
   ...
   // Code added by Vinod Kumar Pal on 27062020
	...
	public function Get_long_desc($pid){
         $this->db->select("*");
   ...
   public function single_industrytype($id){
         $this->db->select("*");
         $this->db->from("tbl_products");
         $this->db->where('id',$id);
         $query=$this->db->get();
